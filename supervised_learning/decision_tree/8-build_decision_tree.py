...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """Find best threshold for one feature using Gini"""
        ...
        # goes_left[i, j] = individu i va à gauche pour seuil j
        goes_left = values[:, None] > thresholds[None, :]

        # is_class[i, k] = individu i est de classe k
        is_class = classes[:, None] == unique_classes[None, :]

        # Left_F[i, j, k] = individu i va à gauche ET est de classe k
        Left_F = goes_left[:, :, None] * is_class[:, None, :]

        # Comptages par classe à gauche et à droite
        left_counts = Left_F.sum(axis=0)
        right_counts = is_class.sum(axis=0)[None, :] - left_counts

        left_total = left_counts.sum(axis=1)
        right_total = right_counts.sum(axis=1)

        # Gini = 1 - sum(proportions²)
        left_gini = 1 - np.sum(
            left_counts ** 2, axis=1) / (left_total ** 2)
        right_gini = 1 - np.sum(
            right_counts ** 2, axis=1) / (right_total ** 2)

        # Moyenne pondérée
        n = values.size
        gini_split = (left_total * left_gini
